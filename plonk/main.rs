...
fn main() {
    ...
    let variable_map: Rc<RefCell<HashMap<String, Vec<Position>>>> =
        Rc::new(RefCell::new(HashMap::new()));
    // type 0: add, type 1: mul, type 2: const
    let circuit_list: Rc<RefCell<Vec<(String, String, String, i32)>>> =
    ...
    gen_circuit(
        ...
        split_list
            .get(0)
            .expect("it will have an equal sign in the middle, at least that's what i think")
            .clone(),
    );
    gen_circuit(
        circuit_list.clone(),
...
fn gen_circuit(
    circuit_list: Rc<RefCell<Vec<(String, String, String, i32)>>>,
    variable_map: Rc<RefCell<HashMap<String, Vec<Position>>>>,
    circuit: String,
) -> bool {
...
fn gen_circuit_add(
    ...
    circuit_list
        .borrow_mut()
        .push((left.to_string(), right.to_string(), result.clone(), 0));
    ...
    if left.len() == 1 {
        ...
        if var_exist {
            variable_map
                ...
                .push(Pos(0, circuit_list.borrow_mut().len() - 1))
        } else {
            variable_map.borrow_mut().insert(
                left.to_string(),
                vec![Pos(0, circuit_list.borrow_mut().len() - 1)],
            );
        }
    }
    if right.len() == 1 {
    ...
    if var_exist {
        variable_map
            ...
            .get_mut(&result)
            .expect("var_exist guaranty its exist")
            .push(Pos(3, circuit_list.borrow_mut().len() - 1))
    } else {
        variable_map.borrow_mut().insert(
            result.clone(),
            vec![Pos(3, circuit_list.borrow_mut().len() - 1)],
        );
    }
    result.clone()
}
...
fn gen_circuit_mul(
    mut circuit_list: Rc<RefCell<Vec<(String, String, String, i32)>>>,
    mut variable_map: Rc<RefCell<HashMap<String, Vec<Position>>>>,
    left: String,
    right: String,
) -> String {
    let left = left.trim();
    ...
    circuit_list
        .borrow_mut()
        .push((left.to_string(), right.to_string(), result.clone(), 1));
    let var_exist = variable_map.borrow_mut().get(left).is_some();
    if var_exist {
        variable_map
            .borrow_mut()
