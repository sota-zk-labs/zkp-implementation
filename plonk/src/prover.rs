...
use ark_poly::{EvaluationDomain, Evaluations, GeneralEvaluationDomain, Polynomial as Poly, UVPolynomial};
...
pub fn generate_proof(compiled_circuit: &CompiledCircuit) -> Proof {
    ...
    let ax = compiled_circuit.gate_constraint().f_ax().clone().add(ax);

    let bx = pre2.mul_by_vanishing_poly(domain);
    let bx = compiled_circuit.gate_constraint().f_bx().clone().add(bx);

    let cx = pre3.mul_by_vanishing_poly(domain);
    let cx = compiled_circuit.gate_constraint().f_cx().clone().add(cx);

    let commitments = commit_round1(&ax, &bx, &cx, &scheme);
    ...
    let slice_poly = SlidePoly::new(tx, domain.size());
    ...
    let bar_s_sigma_1 = compiled_circuit.copy_constraint().get_s_sigma_1().evaluate(&evaluation_challenge);
    let bar_s_sigma_2 = compiled_circuit.copy_constraint().get_s_sigma_2().evaluate(&evaluation_challenge);
    let bar_z_w = z_x.evaluate(&(evaluation_challenge * w));
    let pi_e = compiled_circuit.gate_constraint().pi_x().evaluate(&evaluation_challenge);
    ...
    challenge.digest(&scheme.commit_para(&bar_s_sigma_1));
    challenge.digest(&scheme.commit_para(&bar_s_sigma_2));
    ...
    let w_ev_x = poly_sub_para(&r_x, &bar_r) + poly_sub_para(&ax, &bar_a).mul(v) + poly_sub_para(&bx, &bar_b).mul(v.square())
        + poly_sub_para(&cx, &bar_c).mul(v * v * v) + poly_sub_para(compiled_circuit.copy_constraint().get_s_sigma_1(), &bar_s_sigma_1).mul(v * v * v * v)
        + poly_sub_para(compiled_circuit.copy_constraint().get_s_sigma_2(), &bar_s_sigma_2).mul(v * v * v * v * v);
...
fn compute_acc(beta: &Fr, gamma: &Fr, domain: &GeneralEvaluationDomain<Fr>, compiled_circuit: &CompiledCircuit) -> (Polynomial, Polynomial) {
    ...
    let k1 = compiled_circuit.copy_constraint().k1();
    let k2 = compiled_circuit.copy_constraint().k2();

    for i in 1..compiled_circuit.size {
        let w_i_sub1 = roots.get(i - 1).unwrap();

        let numerator = (compiled_circuit.gate_constraint().f_ax().evaluate(w_i_sub1) + *beta * w_i_sub1 + *gamma)
            * (compiled_circuit.gate_constraint().f_bx().evaluate(w_i_sub1) + *beta * k1 * w_i_sub1 + *gamma)
            * (compiled_circuit.gate_constraint().f_cx().evaluate(w_i_sub1) + *beta * k2 * w_i_sub1 + *gamma);

        let denominator =
            (compiled_circuit.gate_constraint().f_ax().evaluate(w_i_sub1) + *beta * compiled_circuit.copy_constraint().get_s_sigma_1().evaluate(w_i_sub1) + *gamma)
                * (compiled_circuit.gate_constraint().f_bx().evaluate(w_i_sub1) + *beta * compiled_circuit.copy_constraint().get_s_sigma_2().evaluate(w_i_sub1) + *gamma)
                * (compiled_circuit.gate_constraint().f_cx().evaluate(w_i_sub1) + *beta * compiled_circuit.copy_constraint().get_s_sigma_3().evaluate(w_i_sub1) + *gamma);
...
fn compute_quotient_polynomial(beta: &Fr, gamma: &Fr, alpha: &Fr, ax: &Polynomial, bx: &Polynomial, cx: &Polynomial,
    ...
    let line1 = &(ax * bx) * compiled_circuit.gate_constraint().q_mx()
        + ax * compiled_circuit.gate_constraint().q_lx()
        + bx * compiled_circuit.gate_constraint().q_rx()
        + cx * compiled_circuit.gate_constraint().q_ox()
        + compiled_circuit.gate_constraint().pi_x().clone()
        + compiled_circuit.gate_constraint().q_cx().clone();
    ...
    let line3 = (ax.clone() + compiled_circuit.copy_constraint().get_s_sigma_1().mul(*beta) + DensePolynomial::from_coefficients_vec(vec![*gamma]))
        .mul(&(bx.clone() + compiled_circuit.copy_constraint().get_s_sigma_2().mul(*beta) + DensePolynomial::from_coefficients_vec(vec![*gamma])))
        .mul(&(cx.clone() + compiled_circuit.copy_constraint().get_s_sigma_3().mul(*beta) + DensePolynomial::from_coefficients_vec(vec![*gamma])))
        .mul(z_wx)
        .mul(alpha.clone());
...
{
    let mut line1 = compiled_circuit.gate_constraint().q_mx().mul(*bar_a * *bar_b) + compiled_circuit.gate_constraint().q_lx().mul(*bar_a)
        + compiled_circuit.gate_constraint().q_rx().mul(*bar_b) + compiled_circuit.gate_constraint().q_ox().mul(*bar_c)
        + compiled_circuit.gate_constraint().q_cx().clone();
    line1.coeffs[0] += pi_e;

    let line2 = (*bar_a + *beta * eval_challenge + gamma) * (*bar_b + *beta * compiled_circuit.copy_constraint().k1() * eval_challenge + gamma)
        * (*bar_c + *beta * compiled_circuit.copy_constraint().k2() * eval_challenge + gamma) * alpha;
    let line2 = z_x.mul(line2);

    let line3 = (*bar_a + *beta * bar_s_sigma_1 + gamma) * (*bar_b + *beta * bar_s_sigma_2 + gamma) * bar_z_w * alpha;
    let mut tmp2 = compiled_circuit.copy_constraint().get_s_sigma_3().mul(*beta);
    ...
    // check:
    {
        let line22 = (ax.clone() + DensePolynomial::from_coefficients_vec(vec![*gamma, *beta]))
            .mul(&(bx.clone() + DensePolynomial::from_coefficients_vec(vec![*gamma, *beta * compiled_circuit.copy_constraint().k1()])))
            .mul(&(cx.clone() + DensePolynomial::from_coefficients_vec(vec![*gamma, *beta * compiled_circuit.copy_constraint().k2()])))
            .mul(*alpha)
            .mul(z_x);

        let line32 = (ax.clone() + compiled_circuit.copy_constraint().get_s_sigma_1().mul(*beta) + DensePolynomial::from_coefficients_vec(vec![*gamma]))
            .mul(&(bx.clone() + compiled_circuit.copy_constraint().get_s_sigma_2().mul(*beta) + DensePolynomial::from_coefficients_vec(vec![*gamma])))
            .mul(&(cx.clone() + compiled_circuit.copy_constraint().get_s_sigma_3().mul(*beta) + DensePolynomial::from_coefficients_vec(vec![*gamma])))
            .mul(*alpha)
            .mul(z_wx);
